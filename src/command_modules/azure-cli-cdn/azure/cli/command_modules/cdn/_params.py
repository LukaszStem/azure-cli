...
from azure.cli.core.commands.parameters import three_state_flag, tags_type
from azure.cli.core.commands.validators import get_default_location_from_resource_group

from knack.arguments import CLIArgumentType, enum_choice_list, ArgumentsContext
...
def load_arguments(self, args):
    ...
    with self.argument_context('cdn profile create') as c:
        c.argument('sku', **enum_choice_list([item.value for item in list(SkuName)]))
    ...
    with self.argument_context('cdn endpoint') as c:
        ...
        c.argument('is_http_allowed', options_list='--no-http',
        ...
        c.argument('is_https_allowed', options_list='--no-https', 
                   help='Indicates whether HTTPS traffic is not allowed on the endpoint. '
                        'Default is to allow HTTPS traffic.',
                   **three_state_flag(invert=True))
        c.argument('is_compression_enabled', options_list='--enable-compression',
    ...
    # Origin #
    with self.argument_context('cnd origin') as c:
